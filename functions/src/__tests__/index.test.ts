...
describe('Firebase Functions Tests', () => {
  ...
  describe('getDailyReport', () => {
    ...
    it('should use date parameter when provided', async () => {
      ...
      // dateパラメータを使用して実行
      const result = await getDailyReport(mockAxios as unknown as AxiosInstance, mockEsaConfig, '日報/2024/06/20', '2024-06-20');
    ...
    it('should work with date parameter even with mismatched category', async () => {
      const mockPost: EsaPost = {
        body_md: '# 日報\n\n過去の作業内容',
        body_html: '<h1>日報</h1><p>過去の作業内容</p>',
        number: 456,
        name: '日報 2024-06-15',
        tags: ['日報'],
      };
      ...
      // 意図的に異なるカテゴリとdateを指定
      const result = await getDailyReport(mockAxios as unknown as AxiosInstance, mockEsaConfig, '日報/2024/06/20', '2024-06-15');

      // dateパラメータが優先されることを確認
      expect(mockAxios.get).toHaveBeenNthCalledWith(1, '/v1/teams/test-team/posts', {
        params: {
          q: 'on:日報/2024/06/15',
        },
